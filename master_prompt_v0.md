# Masterprompt - ExampleCorp - Log Optimization Agent

We're going to build together a web application prototype called ExampleCorp Log Optimization Agent Terminal Simulator.

Before writing a single line of code, I want you to carefully read the attached context and specifications:
...
- **Components:** Pure Tailwind CSS console-rendered layouts (Strictly NO traditional SaaS dashboards, modern web buttons, floating charts, sidebars, or default web inputs).
...
- **Data engine:** Pure in-memory mock log generator streaming simulated background noise loops (`kubelet`, `kube-proxy`, Prometheus) and target error signatures (`checkout-service-v2` connection pool exhaustion).
